...
_TEST_MODULE_PROMPT = (
    ...
    "- To simulate a service restart (e.g. to verify something persisted), do NOT "
    "call `importlib.reload()` on `harness.module` — it is loaded under a synthetic "
    "module name that reload() cannot resolve and will raise ModuleNotFoundError. "
    "Instead `from conftest import _dispose, _load_app`, then call "
    "`_dispose(harness)` followed by `restarted = _load_app(harness.database_path)` "
    "to get a fresh app instance backed by the same SQLite file, and "
    "`_dispose(restarted)` when done with it.\n"
